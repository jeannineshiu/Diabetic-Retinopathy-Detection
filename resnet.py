...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, stride=1, downsample=None):
        ...
        self.conv2 = nn.Conv2d(out_ch, out_ch, kernel_size=3, stride=stride,
                               padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(out_ch)
        self.conv3 = nn.Conv2d(out_ch, out_ch * self.out_extend, kernel_size=1, bias=False)
        self.bn3 = nn.BatchNorm2d(out_ch * self.out_extend)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
...
class ResNet(nn.Module):
    def __init__(self, block, layers, out_class=5, in_ch=3):
        ...
        self.fc = nn.Linear(512 * block.out_extend, out_class)

    def _customize_layer(self, block, planes, blocks, stride=1):
        # the last layer of a block -> None or perform projection for shortcut
        downsample = None
        ...
        layers = []
        # the first block, eg. conv3_1
        layers.append(block(self.input_size, planes, stride, downsample))
        # increase the dimension only when it comes to the last layer of a bottleneck -> dim = dim * 4
        self.input_size = planes * block.out_extend
        # rest of the blocks, eg. conv3_2, conv3_3, conv3_4
...
def test_accuracy(net, dataset):
    ...
    for index in range(test_size):
        x, gt = dataset[index]
        t_x = torch.unsqueeze(x.clone().detach(), dim=0).type(torch.FloatTensor).to(device)
        pred_y = net(t_x)
...
def main(args):
    ...
    #train_loader = Data.DataLoader(dataset=train_data, batch_size=args.batch, shuffle=False,sampler=ImbalancedDatasetSampler(train_data))
...
# python resnet18_50.py --model resnet18 --batch 4 --weight_decay 0.0005
def get_args():
